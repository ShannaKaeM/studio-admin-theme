...
// suggested products
$has_suggested_products = $storage->get_settings()['features']['suggested-products'];

if ($has_suggested_products) {
	$selectors = [
	...
	foreach ($selectors as $prefix => $selector) {
		...
		blocksy_output_responsive([
			...
			'value' => blocksy_get_theme_mod($prefix . 'products_image_width', $defaults[$prefix]['products_image_width']),
			'unit' => ''
		]);
		...
		blocksy_output_colors([
			'value' => blocksy_get_theme_mod($prefix . 'products_title_color'),
			'default' => [
				'default' => [ 'color' => 'var(--theme-text-color)' ],
				'hover' => [ 'color' => Blocksy_Css_Injector::get_skip_rule_keyword('DEFAULT') ],
			],
		...
		blocksy_output_colors([
			'value' => blocksy_get_theme_mod($prefix . 'products_price_color'),
			'default' => [
				'default' => [ 'color' => Blocksy_Css_Injector::get_skip_rule_keyword('DEFAULT') ],
			],
			'css' => $css,
